...
namespace MonkeyAgent.States.AgentStatets
{
    ...
    class Exploring : IState<StateBasedMonkeyAgent>
    {
        ...
        public void Execute(StateBasedMonkeyAgent obj)
        {
            ...
            switch (goalType)
            {
                case ExploringGoal.Enemy:
                    inRange = obj.ViewedEntities.FindAll(x => x.GetType() != typeof(StateBasedMonkeyAgent) && x is Agent && AIVector.Distance(obj.Position, x.Position) < AIModifiers.maxMeleeAttackRange);
                    if (inRange.Count > 0)
                    {
